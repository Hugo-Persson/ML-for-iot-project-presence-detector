...
MODEL_INPUTS = 828
WEIGHTS_FILE = Path(__file__).parent / "arduino" / "ghost_detector" / "lr_weights.h"
...
COMMANDS = {
    ...
    "r": "Reset weights",
    "s": "Save weights to lr_weights.h",
    "h": "Show help",
}
...
def generate_weights_header(weights: list[float], bias: float) -> str:
    """Generate the lr_weights.h file content."""
    lines = [
        "#pragma once",
        "",
        "// Auto-generated by ghost_detector_cli.py",
        f"constexpr int LR_MODEL_INPUTS = {len(weights)};",
        "",
        f"static const float LR_WEIGHTS_INIT[LR_MODEL_INPUTS] = {{",
    ]

    for i, w in enumerate(weights):
        comma = "," if i < len(weights) - 1 else ""
        lines.append(f"  {w:.8g}f{comma}")

    lines.append("};")
    lines.append("")
    lines.append(f"static const float LR_BIAS_INIT = {bias:.8g}f;")
    lines.append("")
...
class GhostDetectorCLI:
    ...
    def __init__(self, port: str, baud_rate: int = DEFAULT_BAUD_RATE) -> None:
        ...
        self._receiving_weights = False
        self._weight_buffer: list[float] = []
        self._weight_bias: float | None = None
    ...
    def _process_line(self, line: str) -> None:
        ...
        if line == "WEIGHTS_START":
            self._receiving_weights = True
            self._weight_buffer = []
            self._weight_bias = None
            print("\033[96mReceiving weights...\033[0m")
            return

        if self._receiving_weights:
            if line == "WEIGHTS_END":
                self._receiving_weights = False
                self._save_weights()
                return
            if line.startswith("BIAS:"):
                try:
                    self._weight_bias = float(line.split(":")[1].strip())
                except (ValueError, IndexError):
                    print(f"\033[91mFailed to parse bias: {line}\033[0m")
                return
            # Parse weight value
            try:
                weight = float(line)
                self._weight_buffer.append(weight)
                # Show progress every 100 weights
                if len(self._weight_buffer) % 100 == 0:
                    print(f"\r\033[96mReceived {len(self._weight_buffer)}/{MODEL_INPUTS} weights...\033[0m", end="")
            except ValueError:
                pass  # Skip non-numeric lines during weight reception
            return
    ...
    def _save_weights(self) -> None:
        """Save received weights to lr_weights.h."""
        print()  # Clear progress line
        if len(self._weight_buffer) != MODEL_INPUTS:
            print(f"\033[91mError: Expected {MODEL_INPUTS} weights, got {len(self._weight_buffer)}\033[0m")
            return
        if self._weight_bias is None:
            print("\033[91mError: No bias received\033[0m")
            return

        content = generate_weights_header(self._weight_buffer, self._weight_bias)
        try:
            WEIGHTS_FILE.write_text(content)
            print(f"\033[92mWeights saved to {WEIGHTS_FILE}\033[0m")
            print(f"\033[92m  {len(self._weight_buffer)} weights + bias = {self._weight_bias:.8g}\033[0m")
        except OSError as e:
            print(f"\033[91mError saving weights: {e}\033[0m")
...
def main() -> None:
    ...
    if args.send:
        ...
        # For 's' command, use the full processing loop to receive weights
        if args.send == "s":
            print("Waiting for weights from Arduino...")
            cli.start_reading()
            # Wait until weights are received or timeout
            timeout = 30.0  # 30 seconds should be enough for 828 weights
            start = time.time()
            while time.time() - start < timeout:
                if not cli._receiving_weights and len(cli._weight_buffer) > 0:
                    break  # Weights received and saved
                time.sleep(0.1)
            if cli._receiving_weights:
                print("\033[91mTimeout waiting for weights\033[0m")
        else:
